...
<section class="relative mb-12" aria-labelledby="podium-heading">
    ...
    <div class="flex items-end justify-center gap-6 lg:gap-10">
        ...
        {{-- ── 2nd Place: Silver ──────────────────────────────────────────── --}}
        @if($silver)
            <div class="flex flex-col items-center gap-4 flex-1 max-w-[220px]" role="listitem"
                aria-label="2nd place: {{ $silver->name }}, {{ number_format($silver->total_xp) }} XP">
                {{-- Avatar --}}
                <div class="relative">
                    <div
                        class="w-20 h-20 rounded-full bg-gradient-to-br from-slate-300 to-slate-400 flex items-center justify-center text-white font-headline font-extrabold text-2xl shadow-lg ring-4 ring-white">
                ...
                {{-- XP Chip --}}
                <div
                    class="bg-surface-container-high text-on-surface-variant px-4 py-1.5 rounded-full text-xs font-bold font-label">
        ...
        @if($gold)
            <div class="flex flex-col items-center gap-4 flex-1 max-w-[260px]" role="listitem"
                aria-label="1st place: {{ $gold->name }}, {{ number_format($gold->total_xp) }} XP">
                {{-- Crown icon --}}
                <span class="material-symbols-outlined text-4xl text-tertiary" style="font-variation-settings:'FILL' 1;"
                    aria-hidden="true">military_tech</span>
                {{-- Avatar (larger for #1) --}}
                <div class="relative">
                    <div
                        class="w-28 h-28 rounded-full bg-gradient-to-br from-primary to-primary-container flex items-center justify-center text-on-primary font-headline font-extrabold text-3xl shadow-xl shadow-primary/30 ring-4 ring-tertiary-fixed">
                ...
                {{-- XP Chip (gold accent) --}}
                <div
                    class="bg-gradient-to-r from-tertiary-fixed to-tertiary-container text-on-tertiary-fixed px-5 py-1.5 rounded-full text-xs font-black font-label shadow-sm">
        ...
        @if($bronze)
            <div class="flex flex-col items-center gap-4 flex-1 max-w-[220px]" role="listitem"
                ...
                <div class="relative">
                    <div
                        class="w-20 h-20 rounded-full bg-gradient-to-br from-amber-600/70 to-amber-700/50 flex items-center justify-center text-white font-headline font-extrabold text-2xl shadow-lg ring-4 ring-white">
                ...
                {{-- XP Chip --}}
                <div
                    class="bg-amber-50 text-amber-700 px-4 py-1.5 rounded-full text-xs font-bold font-label border border-amber-200/50">
