...
<div class="fixed bottom-0 left-64 right-0 z-40 px-8 pb-4 pointer-events-none" aria-label="Your current rank">
    <div class="pointer-events-auto max-w-4xl mx-auto">
        <div
            class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0_-4px_30px_-8px_rgba(43,75,185,0.18)] border border-outline-variant/10 px-6 py-4 flex items-center gap-6">

            {{-- Rank badge --}}
            <div
                class="flex-shrink-0 flex flex-col items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-primary to-primary-container text-on-primary shadow-lg shadow-primary/25">
                <span
                    class="text-[10px] font-label font-bold uppercase tracking-widest opacity-80 leading-none">Rank</span>
            ...
            <div class="flex-shrink-0 text-right">
                ...
                <p class="text-[10px] font-label font-bold text-on-surface-variant uppercase tracking-widest mt-0.5">
                    Total XP</p>
            </div>
            ...
            @if($nextLevel)
                <div class="flex-shrink-0 hidden sm:flex flex-col items-end gap-1">
                    ...
                    <div class="w-28 h-1.5 bg-surface-container-high rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-primary to-primary-container rounded-full transition-all"
                            style="width: {{ $authUser->progressPercentage() }}%;" role="progressbar"
                            aria-valuenow="{{ $authUser->progressPercentage() }}" aria-valuemin="0" aria-valuemax="100"
                            aria-label="Progress to {{ $nextLevel->name }}"></div>
                    </div>
