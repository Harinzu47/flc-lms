...
<div class="mb-6 flex justify-center" aria-live="polite">
    @if($existingSubmission->status === 'graded')
        <div class="flex flex-col items-center gap-2 bg-secondary-container px-8 py-5 rounded-2xl" role="status"
            ...
            <span
                class="inline-flex items-center gap-1 bg-secondary text-on-secondary px-4 py-1.5 rounded-full text-xs font-bold font-label uppercase">
    ...
    @else
        <div class="flex flex-col items-center gap-3 bg-tertiary-fixed/30 px-8 py-5 rounded-2xl w-full" role="status"
            aria-label="Submission status: Pending Grading">
            <div class="w-14 h-14 bg-tertiary-container/40 rounded-full flex items-center justify-center"
                aria-hidden="true">
            ...
            <span
                class="inline-flex items-center gap-1.5 bg-tertiary-fixed px-5 py-2 rounded-full text-sm font-bold font-label text-on-tertiary-fixed">
            ...
            <a href="{{ route('submissions.download', $existingSubmission) }}"
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-xs text-on-surface-variant truncate"
                        title="{{ $existingSubmission->friendly_file_name }}">
                        {{ $existingSubmission->friendly_file_name }}
                    </p>
                </div>
                <span
                    class="material-symbols-outlined text-sm ml-auto text-on-surface-variant group-hover:text-primary transition-colors flex-shrink-0"
