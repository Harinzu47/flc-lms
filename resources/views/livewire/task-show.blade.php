    ...
    <main class="pt-28 pb-32 px-4">
        <div class="max-w-6xl mx-auto">
            ...
            @if($existingSubmission && $existingSubmission->is_flagged)
                <div class="mb-8 p-5 bg-error-container/20 border-2 border-error/25 rounded-3xl flex items-start gap-4 shadow-sm"
                    role="alert">
                    <div class="w-10 h-10 bg-error-container text-error rounded-2xl flex items-center justify-center flex-shrink-0"
                        aria-hidden="true">
            ...
            <div class="grid grid-cols-12 gap-8 items-stretch mt-8">
                ...
                <div class="col-span-12 {{ $task->deadline ? 'lg:col-span-8' : '' }}">
                    <section class="bg-surface-container-low rounded-3xl p-8 relative overflow-hidden h-full"
                        id="task-instructions" aria-labelledby="instructions-heading">
    ...
    'allow_unsafe_links' => false,
                ...
                @if($task->deadline)
                    ...
                    <div class="col-span-12 lg:col-span-4 flex">
                        <section
                            class="bg-surface-container-lowest rounded-3xl p-8 shadow-sm border border-outline-variant/10 flex flex-col justify-between w-full"
                            aria-label="Deadline information">
                            <div>
                                <p
                                    class="text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-1">
                                    Time Remaining
                                </p>
                                @if($isPast)
                            ...
                            <div class="flex items-center justify-between mt-4">
                                ...
                                <div class="w-16 h-16 relative flex-shrink-0" aria-hidden="true">
                                    <svg class="w-full h-full transform -rotate-90" viewBox="0 0 80 80">
                                        <circle class="text-surface-container-high" cx="40" cy="40" fill="transparent"
                                            r="34" stroke="currentColor" stroke-width="8" />
                                        <circle class="{{ $isPast ? 'text-outline-variant' : 'text-error' }}" cx="40"
                                            cy="40" fill="transparent" r="34" stroke="currentColor" stroke-dasharray="213.6"
                                            stroke-dashoffset="{{ $isPast ? 213 : 53 }}" stroke-linecap="round"
                                            stroke-width="8" />
                                    </svg>
                                    <div class="absolute inset-0 flex items-center justify-center">
                                        <span
                                            class="material-symbols-outlined {{ $isPast ? 'text-outline-variant' : 'text-error' }}">
                                            alarm
                                        </span>
                                    </div>
                                </div>
            ...
            <div class="mt-8" id="task-form">
                <div
                    class="bg-surface-container-lowest rounded-[2rem] p-8 shadow-xl shadow-slate-200/50 border border-white">
                    ...
                    <div class="mt-8 bg-surface-bright rounded-2xl p-5 border border-outline-variant/10"
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-xs font-label font-bold text-on-surface-variant uppercase">Student
                                    Profile</p>
