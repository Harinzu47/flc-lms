{{--
    Task Manager — FLC UMJ Admin Portal
    ────────────────────────────────────────────────────────────────────────────
    Backend:  App\Livewire\Admin\TaskManager
    Layout:   layouts.base
    Pattern:  Mirrors material-manager.blade.php exactly.
    ────────────────────────────────────────────────────────────────────────────
--}}
...
>
    ...
    {{-- ── ADMIN SIDEBAR ────────────────────────────────────────────────────── --}}
    @include('livewire.partials.admin.sidebar', ['activePage' => 'tasks'])
            ...
            <div class="max-w-6xl mx-auto space-y-8">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-on-surface-variant ml-4 text-sm">
                            Manage assignments, essays, and file-upload challenges.
                        </p>
                    </div>
                    <button
                        wire:click="create"
                ...
                <div class="bg-surface-container-lowest rounded-2xl shadow-sm overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm" aria-label="Tasks catalogue">
                            <thead>
                                <tr class="bg-surface-container-low">
                                    ...
                                    <th scope="col" class="text-left px-6 py-4 text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest">Base XP</th>
                                    <th scope="col" class="text-left px-6 py-4 text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest">Deadline</th>
                                ...
                                @forelse($tasks as $task)
                                    <tr class="hover:bg-surface-container-low transition-colors group"
                                        ...
                                        <td class="px-6 py-4">
                                            @php
                                                $typeConfig = [
                                                    'essay'       => ['icon' => 'edit_note',    'bg' => 'bg-primary/10 text-primary'],
                                                    'file_upload' => ['icon' => 'upload_file',  'bg' => 'bg-secondary/10 text-secondary'],
                                                    'quiz'        => ['icon' => 'quiz',         'bg' => 'bg-tertiary/10 text-tertiary'],
                                                ];
                                                $cfg = $typeConfig[$task->type] ?? ['icon' => 'assignment', 'bg' => 'bg-surface-container-high text-on-surface-variant'];
                                            @endphp
                                        ...
                                        {{-- Base XP --}}
                                        <td class="px-6 py-4">
                                            <span class="inline-flex items-center gap-1 font-headline font-black text-tertiary">
                                        ...
                                        <td class="px-6 py-4">
                                            @if($task->deadline)
                                                <div>
                                                    <p class="text-on-surface text-xs font-semibold">
                                            ...
                                            @else
                                                <span class="text-outline-variant text-xs italic">No deadline</span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-6 py-4">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <button
                                                    ...
                                                    wire:confirm="Are you sure you want to delete '{{ addslashes($task->title) }}'? This cannot be undone."
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-16 text-center">
                                            <span class="material-symbols-outlined text-5xl text-outline-variant block mb-3" aria-hidden="true">assignment</span>
                                            <p class="font-headline font-bold text-on-surface">No tasks yet.</p>
                                            <p class="text-on-surface-variant text-sm mt-1">Click "Add New Task" to create your first assignment.</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
    ...
    {{-- ────────────────────────────────────────────────────────────────────────
         CREATE / EDIT MODAL
         Alpine `open` is @entangled with Livewire $isModalOpen.
    ──────────────────────────────────────────────────────────────────────── --}}
    <div
        x-show="open"
        x-cloak
        ...
        >
            ...
            <div class="flex items-center justify-between px-8 py-6 border-b border-outline-variant/10">
                <div class="flex items-center gap-3">
                    ...
                    <h3 id="task-modal-title" class="text-xl font-headline font-extrabold text-on-surface">
            ...
            <div class="px-8 py-6 space-y-5 max-h-[70vh] overflow-y-auto custom-scrollbar">
                ...
                {{-- Title --}}
                <div>
                    <label for="task-title" class="block text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                    ...
                    <input
                        ...
                        wire:model="title"
                        type="text"
                        placeholder="e.g. Analisis Hadits Arbain #3"
                ...
                {{-- Description --}}
                <div>
                    <label for="task-description" class="block text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                        Description <span class="text-error" aria-hidden="true">*</span>
                    </label>
                    <textarea
                        id="task-description"
                ...
                {{-- Type + Base XP (side by side) --}}
                <div class="grid grid-cols-2 gap-4">
                    {{-- Type --}}
                    <div>
                        <label for="task-type" class="block text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                    ...
                    {{-- Base XP --}}
                    <div>
                        <label for="task-xp" class="block text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                            Base XP <span class="text-error" aria-hidden="true">*</span>
                        </label>
                        <div class="relative">
                            <input
                                id="task-xp"
                ...
                {{-- Deadline --}}
                <div>
                    <label for="task-deadline" class="block text-xs font-label font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                        Deadline
                        <span class="normal-case text-outline-variant font-normal ml-1">(optional)</span>
                    </label>
                    <input
                        id="task-deadline"
                        wire:model="deadline"
                        type="datetime-local"
                        class="w-full bg-surface-container-low rounded-xl px-4 py-3 text-sm font-body text-on-surface border-none focus:ring-2 focus:ring-primary/25 transition-all"
                    >
                    @error('deadline')
                        <p class="text-error text-xs mt-1.5 flex items-center gap-1" role="alert">
                            <span class="material-symbols-outlined" style="font-size:13px;" aria-hidden="true">error</span>
